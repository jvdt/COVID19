...
class COVID19:
    ...
    def get_Response(self):
        responseToday           = requests.get(self.link_ecdc_today,stream=True)
        responseYesterday       = requests.get(self.link_ecdc_yesterday,stream=True)
        
        if responseToday.status_code == 200:
            with open(self.file_ecdc_today, 'wb') as f:
                for chunk in responseToday.iter_content(chunk_size=1024):
                    if chunk:
                        self.totalbits += 1024
                        print("Downloaded",self.totalbits*1025, "KB...")
                        f.write(chunk)
            self.df_ecdc = pd.read_excel(self.file_ecdc_today)
        elif responseYesterday.status_code == 200:
            with open(self.file_ecdc_yesterday, 'wb') as f:
                for chunk in responseYesterday.iter_content(chunk_size=1024):
                    if chunk:
                        self.totalbits += 1024
                        print("Downloaded", self.totalbits*1025, "KB...")
                        f.write(chunk)
            self.df_ecdc = pd.read_excel(self.file_ecdc_yesterday)

    def get_GeoId(self):
        df_GeoId = pd.read_excel(self.file_GeoId)
        df_GeoId = df_GeoId.rename(columns={"ISO-3166-alpha2": "GeoId"})
        self.df_ecdc = pd.merge(self.df_ecdc, df_GeoId, on='GeoId')


#    df_ecdc_Continent = df_ecdc.groupby('Continent')['Cases','Deaths'].sum()
    def get_Continent_Data(self):
        self.df_ecdc_Continent = self.df_ecdc[['Continent','Cases','Deaths']].groupby('Continent').sum().sort_values(by='Cases', ascending=False)
        self.df_ecdc_Continent.reset_index(inplace=True)
...
# Dataprep and from here use df_ecdc_basis as startingpoint.
    def produce_plot_1(self):
        ...
        for a in range(0,3): 
            ...
            print(continent)
            df_ecdc_basis=self.df_ecdc[self.df_ecdc['Continent']==continent]
            ...
            self.SubPlot_finalization(ax,df_ecdc_ordered,self.first_InOrder,self.last__Inorder)
            ...
            for i in range(self.first_InOrder,self.last__Inorder): 
                actualloc=df_ecdc_ordered['Countries and territories'].iloc[i]
                df_plot=df_ecdc[df_ecdc['Countries and territories']==actualloc].sort_values(by='DateRep')
                FirstDateC = df_plot[df_plot['Cases']>self.MinimumCases]['DateRep'].min()
                ...
                if(actualloc=='Netherlands'):
                    ax.plot(df_plot['level_0'],cumcases, linewidth=4)
                else:
                    ax.plot(df_plot['level_0'],cumcases) 
            self.SubPlot_finalization(ax,df_ecdc_ordered,self.first_InOrder,self.last__Inorder)
            ax.set_yscale('log')
            ...
            for i in range(self.first_InOrder,self.last__Inorder): 
                actualloc=df_ecdc_ordered['Countries and territories'].iloc[i]
                df_plot=df_ecdc[df_ecdc['Countries and territories']==actualloc].sort_values(by='DateRep')
                FirstDateD = df_plot[df_plot['Deaths']>self.MinimumCases]['DateRep'].min()
                ...
                if(actualloc=='Netherlands'):
                    ax.plot(df_plot['level_0'],cumdeaths, linewidth=4)
                else:
                    ax.plot(df_plot['level_0'],cumdeaths) 
            self.SubPlot_finalization(ax,df_ecdc_ordered,self.first_InOrder,self.last__Inorder)
            ax.set_yscale('log')
            ...
            for i in range(self.first_InOrder,self.last__Inorder): 
                actualloc=df_ecdc_ordered['Countries and territories'].iloc[i]
                df_plot=df_ecdc[df_ecdc['Countries and territories']==actualloc].sort_values(by='DateRep')
                FirstDate5 = df_plot[df_plot['Cases']>self.MinimumCases]['DateRep'].min()
            ...
            for i in range(self.first_InOrder,self.last__Inorder): 
                actualloc=df_ecdc_ordered['Countries and territories'].iloc[i]
                df_plot=df_ecdc[df_ecdc['Countries and territories']==actualloc].sort_values(by='DateRep')
                FirstDate = df_plot[df_plot['Cases']>self.MinimumCases]['DateRep'].min()
